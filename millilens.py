...
# Range of absmagmoth possible
fmoth=fmoth_obs/6600
expfmoth = -2.5*np.log10(fmoth) - 48.60
absmag = expfmoth + 5 - 5*np.log10(Dlummoth)
...
# xl,yl,xh,yh = xmoth-7,ymoth-7,xmoth+7,ymoth+7
...
def NFWlpot(x,y,xpos,ypos,params):
	...
	r = np.sqrt((x-xpos)**2 + (y-ypos)**2)/206264.80624709636 # distance to NFW in radians
...
def NFW(params,r):
        ...
        for i in range(len(xdim)):
                if xdim[i] == 1:
                        sigmanfw.append((2*r_s*rho_s)/3)
                if xdim[i] < 1:
                        sigmanfw.append(((2*r_s*rho_s)/(xdim[i]**2 - 1))*(1 - (2/np.sqrt(1 - xdim[i]**2))*np.arctanh(np.sqrt((1-xdim[i])/(1+xdim[i])))))
                if xdim[i] > 1:
                        sigmanfw.append(((2*r_s*rho_s)/(xdim[i]**2 - 1))*(1 - (2/np.sqrt(xdim[i]**2 - 1))*np.arctan(np.sqrt((xdim[i]-1)/(1+xdim[i])))))
...
def alpx(x,y,xpos,ypos,params):
	b,s,alpha = params 
	q=1
	K = 0

	thetax,thetay = x-xpos,y-ypos # arcsec for now

	return ((b*alpha)/2)*(2*thetax + K*K*thetay)*(s**2 + thetax**2 + (thetay/q)**2 + K*K*thetax*thetay)**((alpha/2)-1)

rdist = 0.080
xdist,ydist = np.sqrt((rdist**2)/2),np.sqrt((rdist**2)/2)
xmil,ymil = xmoth+xdist,ymoth-ydist#xref[0]-0.25, yref[0]+0.25
# rs,rhos = 26.435, 31.0#23.65, 32.84605149#15.82,5000
rs,rhos = 25, 30.0
# xmil,ymil,rs,rhos = -1.74636524 ,16.94734932, 67.42474346+25 ,10.79230981
# sublens1 = np.array([NFWlpot(x,y,xmil,ymil,[rs,rhos]) for x in xrang for y in yrang]).reshape(gridsize,gridsize)
# sublens2 = np.array([NFWlpot(x,y,xmil+0.03,ymil-0.03,[rs,rhos]) for x in xrang for y in yrang]).reshape(gridsize,gridsize)
# sublens = np.array([coreSUB(x,y,xmil,ymil,[0.0003661,3.5e-13]) for x in xrang for y in yrang]).reshape(gridsize,gridsize) # 0.009
# sublens = np.array([coreSUB(x,y,xmil,ymil,[0.01622,3.5e-13]) for x in xrang for y in yrang]).reshape(gridsize,gridsize) # 0.005
# sublens = np.array([coreSUB(x,y,xmil,ymil,[0.0002,3.5e-13]) for x in xrang for y in yrang]).reshape(gridsize,gridsize) # 0.001
# sublens = np.array([alphapot(x,y,xmil,ymil,[1e-11,0.0026,5e-5]) for x in xrang for y in yrang]).reshape(gridsize,gridsize)
# sublens = np.array([alphapot(x,y,xmil,ymil,[1e-12,0.0026,5e-4]) for x in xrang for y in yrang]).reshape(gridsize,gridsize)
# sublens = np.array([alphapot(x,y,xmil,ymil,[1e-13,0.0026,5e-3]) for x in xrang for y in yrang]).reshape(gridsize,gridsize)
# sublens = np.array([alphapot(x,y,xmil,ymil,[0.006125,0.08]) for x in xrang for y in yrang]).reshape(gridsize,gridsize)
sublens = np.array([alphapot(x,y,xmil,ymil,[0.004,0.31]) for x in xrang for y in yrang]).reshape(gridsize,gridsize)
# waveDM = np.zeros((gridsize,gridsize))
# for i in tqdm(range(80)):
# 	perturb = random.choice([0, 1])
# 	xgran,ygran = random.uniform(xlow,xhigh),random.uniform(ylow,yhigh)
# 	sublens = np.array([NFWlpot(x,y,xgran,ygran,[rs,rhos]) for x in xrang for y in yrang]).reshape(gridsize,gridsize)
# 	if perturb==0:
# 		waveDM+=sublens
# 	if perturb==1:
# 		waveDM+=sublens

# waveDMkap = np.loadtxt('waveDMfield.txt',usecols=2).reshape((gridsize,gridsize))
# totlpot = np.genfromtxt('waveDMlpot.txt')
...
detmagtot,totkap = magnmap(totlpot,xrang,yrang)
# totkap=avgkap+waveDMkap
fig3,ax3=subplots(1,figsize=(15,15),sharex=False,sharey=False,facecolor='w', edgecolor='k')
im3 = ax3.imshow(detmagtot,extent=(xlow,xhigh,ylow,yhigh),aspect='auto',cmap='seismic',origin='lower',norm=colors.CenteredNorm())
milliCC = ax3.contour(x,y,detmagtot,levels=[0],colors='g',linestyles='dashed')
# ax3.contour(x,y,detmagtot,levels=1000,colors='b',linestyles='dashed')
macroCC = ax3.contour(x,y,detmag,levels=[0],colors='y',linestyles='dashed')
ax3.scatter(xref,yref,color='yellow',marker='^',label='Observed')
ax3.scatter(xmoth,ymoth,color='g',marker='>',label='Mothra',alpha=0.5)
plt.xlim(xl,xh)
plt.ylim(yl,yh)
ax3.invert_xaxis()
...
totkapinterp = RectBivariateSpline(xrang,yrang,totkap.T)
avgkapinterp = RectBivariateSpline(xrang,yrang,avgkap.T)
diffkapinterp = RectBivariateSpline(xrang,yrang,totkap.T-avgkap.T)
magn = 1/detmaginterp.ev(xmoth,ymoth)

print(magn)

densmoth = avgkapinterp.ev(xmil,ymil)*sigcrit
densdiff = diffkapinterp.ev(xmil,ymil)*sigcrit
...
RMSCC = np.sqrt(np.mean((xccmilrang-xccmacrang)**2))
...
def fims2(x,y,tdsurf):
	...
	contourx=ax2.contour(x,y,np.gradient(tdsurf)[0]/(nx/206264.80624709636),levels=[0],origin='lower',colors='pink')
	contoury=ax2.contour(x,y,np.gradient(tdsurf)[1]/(nx/206264.80624709636),levels=[0],origin='lower')
...
# xinit,yinit = fims2(x,y,avgtdsurf)
xrecon,yrecon = fims2(x,y,avgtdsurf)
# ax3.scatter(xinit,yinit,color='k',marker='*',alpha=0.5)
ax3.scatter(xrecon,yrecon,color='k',marker='*',label='Recon.')
...
magnb0tot,magnb1tot = 1/detmaginterp.ev(xrec[0],yrec[0]),1/detmaginterp.ev(xrec[1],yrec[1])
print(magnb0,magnb1)
print(magnb0tot,magnb1tot)

plt.show()

# factor = 534526963912.9919
# totkap = totkap*factor
fig1,ax1=subplots(1,figsize=(17,17),sharex=False,sharey=False,facecolor='w', edgecolor='k')
im1 = ax1.imshow(totkap,extent=(xlow,xhigh,ylow,yhigh),aspect='auto',cmap='Greys',origin='lower')
contourmass=ax1.contour(x,y,totkap,levels=90,origin='lower',alpha=0.5,zorder=2)
ax1.contour(x,y,detmagtot,levels=[0],colors='g',linestyles='dashed')
ax1.scatter(xrecon,yrecon,color='k',marker='*',label='Recon.')
...
ax1.scatter(xref,yref,color='yellow',marker='^',label='Observed')
ax1.scatter(xmoth,ymoth,color='g',marker='>',label='Mothra',alpha=0.5)
ax1.set_xlim(xl,xh)
ax1.set_ylim(yl,yh)
ax1.invert_xaxis()
...
xclump,yclump,massclump = region(xmil,ymil,0.00401,(totkap-avgkap)*sigcrit)
print(massclump)

ax1.legend(loc=3,title='',ncol=3,prop={'size':9})

plt.show()
nfw_to_mothra = np.sqrt((xmil-xmoth)**2 + (ymil-ymoth)**2)

def BIC(params,y,dy,N,lpot):
	...
	# nfwlens = np.array([NFWlpot(i,j,xsub,ysub,[r_s,rho_s]) for i in xrang for j in yrang]).reshape(gridsize,gridsize)
	# totlpot = avglpot+nfwlens
	tottdsurf = ((1+zd)/c_light)*((Ddpc*Dspc)/Ddspc)*pcconv*(0.5*thetabeta - lpot.T)
	try:
	...
	loglikelihood = -0.5*sum((((xref - xrec)**2)/(dy[0]**2)) + (((yref - yrec)**2)/(dy[1]**2)) + (((magnmoth-magn)**2)/(dy[2]**2)) 
				+ (((magnb0-magnarc0)**2)/(dy[3]**2)) + (((magnb1-magnarc1)**2)/(dy[3]**2)) + ((max(xccmilrang-xccmacrang)**2)/(dy[4]**2)))
					# + ((sum((xccmilrang-xccmacrang)**2))/(dy[4]**2)))
...
dyobs=np.array([0.04,0.04,600,1,0.1])
xfield,yfield = np.meshgrid(xrang,yrang)
bic,aic = BIC([rs,rhos],[np.array(xref),np.array(yref),xmil,ymil],dyobs,6000,totlpot)
...
# 	# 	integ = r*kappatstrip(r,params)
# 	# 	return integ
# 	mass = 2*np.pi*integrate.quad(integrand,1e-10,rlim)[0]
# 	return mass # Solar masses
# def massfuck(r,params):
# 	b,D = params
# 	a = 0.005
# 	num = r**2
# 	dem = ((r**2)/b) + r*(1+(a/b)) + a
# 	return num/dem
# rrang = np.linspace(1e-4,1000,100000) # arcsec
# kappamod = kappatstrip(rrang,[1,3.50941244e-13])
# dkap= np.diff(np.log10(kappamod))/np.diff(np.log10(rrang))
# xderiv = (np.log10(rrang)[:-1] + np.log10(rrang)[1:]) / 2
# drint = interp1d(dkap,xderiv)([-1])[0] # radius at dlog(kappa)/dr = -1 (isothermal)
# rcore = 10**drint # in arcsec
# masscore = masststrip(rcore,[1,3.50941244e-13])
# print(rcore*arcsec_pc)
# print(masscore)
# print(sum(np.array([massfuck(r,[1,3.50941244e-13]) for r in rrang if r<=rcore])))
# # plt.figure()
# # plt.plot(np.log10(rrang),np.log10(kappamod))
sys.exit()
...
nbins=100
binvals = np.linspace(1e-4,0.09,nbins) # Values of bins in arcsec
densprof,stddensprof = circprof(xmil,ymil,binvals,(totkap-avgkap)*sigcrit)
########## PLOT ##################
...
rcore = 10**drint # in arcsec
print(rcore*arcsec_pc)
xclump,yclump,massclump = region(xmil,ymil,rcore,(totkap-avgkap)*sigcrit)
print(massclump)
